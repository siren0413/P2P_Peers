...
public class ServerHSQLDB {
	/**
	 * Database for index server There are two tables in there: 'PeerInfo' to
	 * save peer registry information and 'FileInfo' for peer files management.
	 */
	...
	public static void initDB() {
		...
		try {
			...
			String createPeerInfo = "CREATE TABLE       " + peerInfoTable + " (" + "id         VARCHAR    NOT NULL           primary key,"
					+ "ip         VARCHAR      NOT NULL			," + "port       VARCHAR         NOT NULL, " + " constraint peer_id_unique UNIQUE ( ip )" + ")";

			String createFileInfo = "CREATE TABLE       " + fileInfoTable + "  (" + " peer_id  		VARCHAR 	NOT NULL, " + "	file_name 	VARCHAR 	NOT NULL,"
					+ " FOREIGN KEY (peer_id) REFERENCES " + peerInfoTable + " ( id ) ," + " constraint both_unique UNIQUE ( peer_id,file_name ) )";
